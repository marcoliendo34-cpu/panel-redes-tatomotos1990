...
// Una pieza puede ir a varias redes. Las piezas antiguas guardaban una
// sola en `network`; las nuevas usan la lista `networks`. Esta función
// devuelve siempre una lista, venga como venga.
export function redesDe(post) {
  if (!post) return []
  if (Array.isArray(post.networks) && post.networks.length > 0) return post.networks
  return post.network ? [post.network] : []
}

// Texto listo para mostrar: "Instagram · TikTok"
export function redesTexto(post) {
  const redes = redesDe(post)
  if (redes.length === 0) return '—'
  return redes.map((r) => NETWORK_LABEL[r] || r).join(' · ')
}
